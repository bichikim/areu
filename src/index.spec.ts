...
describe('are-u', () => {
  ...
  describe('object', () => {
    it('can validate object type and members', () => {
      ...
      expect(validate({
        ...
        age: 900,
        isMale: true,
      }, schemaHasMembers)).to.equal(true)
      expect(validate({
        name: 'bichi',
        age: '900',
        isMale: 'true',
      }, schemaHasMembers)).to.equal(false)
    })
  ...
  describe('array', () => {
    it('can validate array type and members', () => {
      ...
      expect(validate(['the', 'super', 'easy', 'and', 'fast', 'validation'],
        schemaMembers)).to.equal(true)
      expect(validate(['the', 'super', 'easy', 'and', 'fast', 'validation', 2],
        schemaMembers)).to.equal(false)
      const schemaMembersMoreTypes = array([string(), number(), any().required()])
      expect(validate(['the', 'super', 'easy', 'and', 'fast', 'validation'],
        schemaMembersMoreTypes)).to.equal(false)
      expect(validate(['the', 2, false, 'super', 'easy', 'and', 'fast', 'validation'],
        schemaMembersMoreTypes)).to.equal(true)
      expect(validate(['the', 2],
        schemaMembersMoreTypes)).to.equal(false)
    })
    it('can validate deep', () => {
      ...
      expect(validate([{name: 'bichi', age: 9999}, true],
        schema)).to.equal(true)
      expect(validate([{name: 'bichi', age: 9999}],
        schema)).to.equal(false)
      expect(validate([{name: 'bichi', age: 'unknown'}],
        schema)).to.equal(false)
    })
  })
})
